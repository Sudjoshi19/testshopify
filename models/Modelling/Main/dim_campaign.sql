...
{% for i in results_list %}
    ...
    select 
        {{ dbt_utils.surrogate_key(['campaign_id','campaign_type','campaign_name','ad_channel',
        'adgroup_id','adgroup_name','ad_id','ad_name'])}} 
        AS campaign_key,
    campaign_id, campaign_type, campaign_name,ad_channel, 
    adgroup_id,adgroup_name,ad_id,ad_name,
    {% if var('timezone_conversion_flag') %}
    ...
    {% else %}
        ...
        null as run_id,
    {% endif %}
    row_number() over(partition by campaign_id,campaign_type, campaign_name,adgroup_id,ad_id,adgroup_name,ad_name 
